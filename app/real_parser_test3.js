...
// --- Test harness using the REAL parseMboxEmailsStreaming verbatim ---
const mboxPath = process.argv[2];
if (!mboxPath) { console.log('Usage: node real_parser_test3.js <mbox-path>'); process.exit(1); }

const results = [];
parseMboxEmailsStreaming(mboxPath, async (batch) => {
  for (const e of batch) results.push(e);
}, { batchSize: 200 }).then((total) => {
  console.log('Total processed:', total);
  console.log('Results collected:', results.length);
  results.forEach((e, i) => {
    console.log(`[${i}] messageId=${JSON.stringify(e.messageId)} subject=${JSON.stringify(e.subject)} bodyLen=${e.body ? e.body.length : 0}`);
  });
}).catch(err => {
